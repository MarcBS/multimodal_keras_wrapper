...
class Staged_Network(object):
        ...
        if(not reloading_model):
            ...
            if(isinstance(stage, list)):
                ...
                for b in range(nBranches):
                    ...
                    stage[b].setName(model_name, plots_path=self.plot_path+'/'+model_name, models_path=self.model_path+'/'+model_name)
            else:
                training_is_enabled = [training_is_enabled]
                expand_dimensions = [expand_dimensions]
                balanced = [balanced]
                model_name = 'Stage_'+ str(nStages) +'/Branch_0'
                stage.setName(model_name, plots_path=self.plot_path+'/'+model_name, models_path=self.model_path+'/'+model_name)
    ...
    def addBranch(self, branch, stage_id, axis=0, out_name=None, in_name=None, 
        ...
        for b in range(nBranches):
            ...
            branch[b].setName(model_name, plots_path=self.plot_path+'/'+model_name, models_path=self.model_path+'/'+model_name)
    ...
    def removeBranches(self, branch_ids, stage_id):
        ...
        for b in range(len(self.__stages)):
            ...
            self.__stages[stage_id][b].setName(model_name, plots_path=self.plot_path+'/'+model_name, models_path=self.model_path+'/'+model_name)
        ...
        for s_, s in enumerate(self.__stages):
            if(isinstance(s, list)):
                for b_, b in enumerate(s):
                    ...
                    b.setName(model_name, plots_path=self.plot_path+'/'+model_name, models_path=self.model_path+'/'+model_name, clear_dirs=clear_dirs)
            else:
                model_name = 'Stage_'+ str(s_) +'/Branch_0'
                s.setName(model_name, plots_path=self.plot_path+'/'+model_name, models_path=self.model_path+'/'+model_name, clear_dirs=clear_dirs)
        ...
        for it_test in range(numIterationsTest):
            ...
            for i_net, net in enumerate(stage):
                if(len(stage) == 1): # only one model
                    ...
                    result = net.testOnBatch(X_in, Y_test, accuracy=True, out_name=self.__outNames[id_last_stage][i_net])
                if(result):
                    ...
                    if(training_is_enabled[i_net]):
                        if(len(stage) == 1): # single stage model
                            ...
                            result = net.trainOnBatch(X_in, Y_batch, batch_size=params['batch_size'], 
                                                     out_name=self.__outNames[stage_id], balanced=self.__balancedTraining[stage_id][0])
                        else: # branched model
                            # Select input
                            ...
                            result = net.trainOnBatch(X_in, Y_batch, batch_size=params['batch_size'], 
                                                     out_name=self.__outNames[stage_id][i_net], balanced=self.__balancedTraining[stage_id][i_net])
                        if(result):
                        ...
                        # Report train info
                        if(state['count_iteration'] % params['report_iter'] == 0 and losses_train[i_net]): # only plot if we have some data
                            ...
                            top_score = np.sum(np.array(top_scores_train[i_net]) * np.array(counts_batch[i_net])) / n_report
        ...
        for s in range(stage_id):
            ...
            if(isinstance(stage, list)):
                ...
                for i, net in enumerate(stage):
                    if(isinstance(net.model, Graph)):
                        o = net.predictOnBatch(X, in_name=self.__inNames[s][i], out_name=self.__outNames[s][i], expand=self.__expandDimensions[s][i])
                    else:
            ...
            # Single model stage
                ...
                if(hasattr(stage, 'model') and isinstance(stage.model, Graph)):
                    if(s == self.getNumStages()-1): # only apply the out_name if we are at the final stage of the Staged_Network
                        out_name = self.__outNames[s]
                    else:
                        out_name = None
                    X = stage.predictOnBatch(X, in_name=self.__inNames[s], out_name=out_name, expand=self.__expandDimensions[s][0])
                else:
    ...
    def _getWorsePairs(self, conf_mat, N, avoid_pairs):
        ...
        # Normalize conf_mat
        norm_conf_mat = conf_mat/np.repeat(np.transpose(np.expand_dims(np.sum(conf_mat,axis=1),0)),n_classes,axis=1).astype(np.float32)
